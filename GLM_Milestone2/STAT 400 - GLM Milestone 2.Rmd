...
Among predictors, strong internal correlations also appear — for example, REB is highly correlated with DREB (0.80), and FG_PCT is moderately correlated with PTS (0.40). These relationships indicate some redundancy among variables, meaning multicollinearity may need to be considered later when selecting predictors for the final GLM. Overall, the heatmap supports the idea that offensive efficiency, point differential, and scoring are the strongest statistical drivers of team success.


## Statistical Analysis

From the previous plot, there is very strong evidence of multicollinearity in a lot of the predictors that exist in the entire predictor set we are using. For this reason, we opt to use the `cv.glmnet()` function in the `glmnet` package to perform variable selection using LASSO regression with 5 fold cross validation as a preliminary model. 

```{r}
# Predict if team goes to playoffs based on game statistics alone, this one does 
# not include the year and team because they act more like level 2 grouping variables
# and would be more appropriate to use for a mixed effects model.

X = model.matrix(PlayoffAppearanceIndicator ~ . - SEASON_YEAR - TEAM_NAME, data = NBATeamData)[, -1]

y = NBATeamData$PlayoffAppearanceIndicator
...
For this particular model, we report a mean min AUC of 0.9822 and a mean 1se AUC of 0.9793 with 5 fold cross validation, which indicates that the variables `FGA`, `FTM`, `WINPCT`, and `CONFERENCE`, are the strongest predictors of playoff appearance. However, given the very large coefficient on WINPCT, we have good reason to believe that it is a proxy for playoff appearance.

For a more balanced approach, we will consider a model without `WINPCT`, given that it is a very strong proxy for predictng if a team makes it to the playoffs or not. 

```{r}
X2 = model.matrix(PlayoffAppearanceIndicator ~ . - SEASON_YEAR - TEAM_NAME - WINPCT, data = NBATeamData)[, -1]

y2 =  NBATeamData$PlayoffAppearanceIndicator
...
When we do not consider the teams win percentage, `FGA`, `FTM`, `OREB`, and `PLUS_MINUS` are the nonzero predictors. Still, the model is quite strong, with a mean min AUC of 0.9616 and a mean 1se AUC of 0.9592. 

To test if these predictors are statistically significant, we will fit a standard logistic regression model with the previous Predictors and look at the standard errors.

```{r}
glm1 = glm(PlayoffAppearanceIndicator ~ FGA + FTM + OREB + PLUS_MINUS, family = binomial, data = NBATeamData)
...
At the $\alpha = 0.05$ significance level, only `PLUS_MINUS` is a significant predictor of playoff appearance when the remaining variables are included in the model. When checking for variance inflation factors, the predictors are all around 1 so we can say that collinearity is not a problem for this model.  
