...
Next we want to add one of our variables of interest teaching_method to create a random slopes and intercepts model

```{r}
RandomSlopeInterceptModel <- 
  lmer(posttest ~ teaching_method + (1 + teaching_method | classroom),
                              data = testScores,
                              REML = FALSE)

summary(RandomSlopeInterceptModel)
```
Model Representation:
Level 1: $$Yij = Ai + bi*Xij,standard + Eij$$
...
Interpretation: This model is the same as the last model, with the added level 1 variable pretest. This means there is still an error term associated with teaching method's slope and intercept between classes, but pretest will not add any extra error terms since it is a level 1 variable and is not a classroom level variable. For this model, there is an intercept term of 41.23486, a slope term for pretest of 0.56828, and a slope term for standard teaching method of -7.56567. All 3 terms have t-values with an absolute value above 2 and should be considered significant in the model. An intercept of 41.23486 means that with a pretest score of 0 and a teaching method of experimental, we should expect a posttest score of 41.23486. A pretest slope of 0.56828 means that with teaching method held constant, an increase in 1 for pretest score has an expected 0.56828 increase in posttest score(the better a student does on the pretest, the better they will do on the posttest, on average). A teaching method slope of -7.56567 means that with pretest held constant, a student in a standard classroom is expected to score 7.56567 points worse on their posttest.




```{r}
RandomInterceptModelPretest <- lmer(posttest ~ pretest + teaching_method + (1 | classroom),
                              data = testScores,
                              REML = FALSE)

summary(RandomInterceptModelPretest)
```
Model Representation:
Level 1: $$Yij = Ai + bi*Xij,standard + bj*Xpretest+ Eij$$
...
Interpretation: This model removes the slope error term from the previous model, which means that each classroom model is given the same slope associated with the teaching method. This model has an intercept estimate of 41.23589, a pretest slope estimate of 0.56827, and a standard teaching method slope estimate of -7.56591. These estimates are nearly identical to our previous model, and the interpretations remain the same.



Finally we need to compare all our possible models to see which most accurately predicts posttest score using pretest, which we will compare using AIC since we used maximum likelihood estimation to derive our models.

```{r}
AIC(RandomInterceptsModel)
AIC(RandomSlopeInterceptModel)
AIC(RandomSlopeInterceptModelPretest)
AIC(RandomInterceptModelPretest)
```

When comparing all 4 models, the 2 best models by AIC are the random slope random intercept model including pretest and teaching method with an AIC of 10965.55, and the simplified random intercept model including pretest and teaching method with an AIC of 10961.56. Since the AIC values are so close between these 2 models, there is no major preference of final model between these 2, but since the random intercepts model with pretest and teaching method has a slightly lower AIC as well as being more simplistic, that is our final model.
